...
# 1. FORCE PAGE CONFIG FIRST BEFORE ANY HEAVY IMPORTS
...
# 3. INTERACTIVE GRAPHICS & ML (Wrapped cleanly)
try:
    import plotly.express as px
    import plotly.graph_objects as go
except ModuleNotFoundError:
    st.error("🔄 Streamlit Cloud is still installing the required graphic packages. Please click 'Reboot App' in the right-side menu panel to finish initialization.")
    st.stop()

from sklearn.model_selection import train_test_split
from sklearn.ensemble import RandomForestClassifier

# 4. MANUALLY INJECT STYLE SHEET WITH SAFETY CHECK
try:
    with open("style.css") as f:
        st.markdown(f"<style>{f.read()}</style>", unsafe_allow_html=True)
except FileNotFoundError:
    pass # Fallback smoothly if style sheet isn't tracked yet
...
@st.cache_data
def load_data():
    # Use latin1 encoding to completely bypass the unicode character errors
...
try:
    ...
    st.error(f"Data Core Error: Make sure 'European_Bank.csv' is uploaded to your GitHub repository. Error details: {e}")
    st.stop()
...
with left_pane:
    ...
    # Process input matching model dimensions
